...
export const Phase5p2 = () => {
    ...
    const question = 'Do you want a dog or a cat? ';
    ...
    async function handleDragEnd({ active, over }) {
        ...
        if (draggedCard) {
            if (draggedCard.id === 'dog' || draggedCard.id === 'cat') {
                ...
                setTimeout(() => navigate('/phase5p3'), 1000);
            }
            else {
                setIncorrect(true);
                await onSound("The question is 'do you want a dog or a cat. Please try again!");
                return;
            }
                            ...
                            .map(c => (
                                <DraggableCard
                                    ...
                                    style={{
                                        ...
                                        top: `${c.pos.yPct}%`,
                                        position: 'absolute',
                                        outline: (incorrect && (c.id === 'dog' || c.id === 'cat')) ? '3px solid red' : 'none',
                                        outlineOffset: (incorrect && (c.id === 'dog' || c.id === 'cat')) ? '3px' : '0',
                                        backgroundColor: (incorrect && (c.id === 'dog' || c.id === 'cat')) ? 'rgba(255,0,0,0.15)' : 'transparent',
                                        boxShadow: (incorrect && (c.id === 'dog' || c.id === 'cat')) ? '0 0 12px 4px rgba(255,0,0,0.6)' : 'none',
                                        borderRadius: '8px',
                                    }}
